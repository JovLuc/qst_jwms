	...
	<div id="wrap">
		...
		<div class="leftMeun" id="leftMeun">
			...
			<div class="meun-item meun-item-active">
				<img src="${pageContext.request.contextPath}/images/icon_user_grey.png">可选课程
			</div>
			<div class="meun-item">
		...
		<div id="rightContent">
            ...
            <div class="tab-content">
            	...
            	<div role="tabpanel" class="tab-pane active" id="sour">
                	...
                	//sql语句,遍历整个admin表的总数
                	String sql = "select count(*) from kebiao k,kecheng c,laoshi l where k.kecheng_id=c.id and k.laoshi_id=l.id";
                	//调用DB里面的方法
                	ps = db.getPs(sql);       	               	
                	//执行sql语句,并获得返回的结果集
                	rs = ps.executeQuery();
					...
					<div class="data-div">
						<div class="row tableHeader">
							...
							<div class="col-lg-1 col-md-1 col-sm-1 col-xs-1 ">课程名称</div>
                		...
                		%>
							<div class="row">
								...
								<div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
									<button class="btn btn-danger btn-xs"><a style="color: white;" href="${pageContext.request.contextPath}/SelectKeBiao?id=<%=rs.getInt("id")%>&oldName=<%=oldName%>&oldId=<%=oldId%>">选择课程</a></button>
									&nbsp;
								</div>
							</div>
						<%
					...
					<ul class="pagination">
						...
						<li>
							...
							<a href="kebiaoSelect.jsp?oldName=<%=oldName %>&oldId=<%=oldId %>&page=<%=(p-1)%>"><i class="glyphicon glyphicon-menu-left"> </i></a>
							<%
								}
							%>
						</li>
						<li>
							<%
								if(p < page_count){
							%>							
							<a href="kebiaoSelect.jsp?oldName=<%=oldName %>&oldId=<%=oldId %>&page=<%=(p+1)%>"><i class="glyphicon glyphicon-menu-right"> </i></a>
							<%
								}
							%>
						</li>
