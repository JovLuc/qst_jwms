	...
	<div id="wrap">
		...
		<div class="leftMeun" id="leftMeun">
			...
			<div class="meun-item meun-item-active">
				<img src="${pageContext.request.contextPath}/images/icon_card_grey.png">学生信息
			</div>
			<div class="meun-item">
		...
		<div id="rightContent">
            ...
            <div class="tab-content">
            	...
            	<div role="tabpanel" class="tab-pane active" id="sour">
                	...
                	//sql语句,遍历整个admin表的总数
                	String sql = "select count(*) from xuesheng";
                	//调用DB里面的方法
                	ps = db.getPs(sql);       	               	
                	//执行sql语句,并获得返回的结果集
                	rs = ps.executeQuery();
					...
					<div class="data-div">
						<div class="row tableHeader">
							...
							<div class="col-lg-1 col-md-1 col-sm-1 col-xs-1 ">学号</div>
							...
							<div class="col-lg-1 col-md-1 col-sm-1 col-xs-1 ">班级</div>
                		...
                		%>
							<div class="row">
								...
								<div class="col-lg-1 col-md-1 col-sm-1 col-xs-1  levl3">
									<span class=""> &nbsp;</span><span><%=rs.getString("xuehao") %></span>
								</div>
								...
								<div class="col-lg-1 col-md-1 col-sm-1 col-xs-1  levl3">
									<span class=""> &nbsp;</span><span><%=rs.getString("banji") %></span>
								</div>
								...
								<div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
									...
									<button class="btn btn-danger btn-xs"><a style="color: white" href="${pageContext.request.contextPath}/DeleXueSheng?id=<%=rs.getInt("id")%>&oldName=<%=oldName%>&oldId=<%=oldId%>">删除</a></button>
								</div>
							</div>
						<%
					...
					<ul class="pagination">
						...
						<li>
							...
							<a href="xueshengMes.jsp?oldName=<%=oldName %>&oldId=<%=oldId %>&page=<%=(p-1)%>"><i class="glyphicon glyphicon-menu-left"> </i></a>
							<%
								}
							%>
						</li>
						<li>
							<%
								if(p < page_count){
							%>							
							<a href="xueshengMes.jsp?oldName=<%=oldName %>&oldId=<%=oldId %>&page=<%=(p+1)%>"><i class="glyphicon glyphicon-menu-right"> </i></a>
							<%
								}
							%>
						</li>
						...
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								<div class="modal-header">
									...
									<h4 class="modal-title" id="gridSystemModalLabel">编辑学生信息</h4>
								</div>
								<div class="modal-body">
									<div class="container-fluid">
										<form class="form-horizontal" id="formEdi" action="${pageContext.request.contextPath}/EdiXueSheng" method="post">
											...
											<div class="form-group ">
												...
												<div class="col-xs-8 ">
													<input type="" class="form-control input-sm duiqi" name="xuehao" id="xuehao" >
											...
											<div class="form-group">
												<label for="userPw" class="col-xs-3 control-label">班&nbsp;&nbsp;级：</label>
												<div class="col-xs-8 ">
													<input type="" class="form-control input-sm duiqi" name="banji" id="banji" >
												</div>
											</div>
											<div class="form-group">
...
function Edi() {
	...
	if (xuehao == '' || xingming == '' || nianling == '' || id == '' || banji == '' || loginname == '' || loginpw == '') {
		alert('请填写完整！！！');
	} else {
